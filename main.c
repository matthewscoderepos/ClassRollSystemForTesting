...
//Saves the student array into the students.csv file in CSV format. 
...
//Checks if the student matching a UID exists in the student array
...
int main()
{
	//make room for 100 students. This can be increased or decreased.
	...
	if (fp != NULL)
	{
		//While fgets does not read in a NULL line
		while (fgets(line, sizeof line, fp) != NULL)
		{
			char *rest = line;
			token = "";
			//While there are tokens left in the line (values between ",")
			while ((token = strdup(strtok_r(rest, ",", &rest))))
			{
				//Read in the data to the correct values of the student struct in the students array
				switch (i)
				{
				case 0:
				{
	...
	while (1)
	{
		...
		//fflush is required to clear out newlines still in the buffer
		fflush(stdin);
		...
		switch (option)
		{
			...
			case 2:
			{
				//Search for a student by uid, name, or email
				char search[40];
			...
			case 3:
			{
				...
				if (info[0] != '\n')			 
				{
					...
					if (num == 5 && (strstr(info, ",,") == NULL) && (strstr(info, ",\n") == NULL) && info[0] != ',')
					{
						...
						while ((token = strdup(strtok_r(rest, ",", &rest))))
						{
							//Add the data if it is formatted correctly, else goto error message
							switch (i)
							{
								case 0:
								{
								...
								case 2:
								{
									if (strstr(token, "@") != NULL && strlen(token) <= 40)
										students[count].email = strdup(token);
									else
										goto WRONGADD;
									break;
			...
			case 5:
			{
				...
				for (int j = 0; j < count; j++)
				{
					...
					if (!strcmp(input, students[j].usfid))
					{
						...
						//Switch on entered selection, overwrite the chosen data is the formatting is correct
						switch (atoi(input))
						{
							case 1:
							{
							...
							case 3:
							{
								if (strstr(newData, "@") != NULL && strlen(newData) < 40)
